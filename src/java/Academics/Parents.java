    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (teacherid != null) {
            try {
                ...
                String query = "Insert into parent(id,name,pwd,gender,dob,telephone,mobile,course,email,father,occupation,present,permanent,state,city)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                ...
                j = pst.executeUpdate();
                if (j > 0) {
                    connection.AutoID.updateCampaignId("parent", teacherid);
                    response.sendRedirect("Admin/Parent.jsp");
                    session.setAttribute("msg", "New Parent is Successfully Registered!!");
                } else {
                    response.sendRedirect("Admin/Parent.jsp");
                    session.setAttribute("msg", "New Parent Not Ragistered!!");
                }
