    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (teacherid != null) {
            try {
                ...
                String query = "Insert into admin_reg(id,name,pwd,gender,dob,telephone,mobile,course,email,father,occupation,present,permanent,state,city)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                ...
                j = pst.executeUpdate();
                if (j > 0) {
                    connection.AutoID.updateCampaignId("Admin", teacherid);
                    response.sendRedirect("Admin/Regi_admin.jsp");
                    session.setAttribute("msg", "New Admin is Successfully Registered!!");
                } else {
                    response.sendRedirect("Admin/Regi_admin.jsp");
                    session.setAttribute("msg", "New Admin Not Ragistered!!");
                }
